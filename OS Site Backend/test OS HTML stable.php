...
function generateCalendarDates($startDay, $numDays, $monthNumber) {
    ...
    <div class="row">
        <span class="col text-center align-content-center" style="display: inline">
            <button type="button" class="btn btn-primary" onclick="showPrev()"><-  </button><span style="font-size: 32px">  May 2023  </span><button type="button" class="btn btn-primary">  -></button>
            <br>
            <br>
        </span>
    </div>
    ...
    for ($row = 1; $row <= 6; $row++) {
        ...
        for ($col = 1; $col <= 7; $col++) {
            if ($skipDays > 0) {
                ...
                $date = date('Y-m-d', strtotime("2023-0".$monthNumber."-$currentDay")); // Date Formatting for SQL
